...
def run_setup(with_binary):
    if with_binary:
        ...
        extra_compile_args = {
            ...
            'linux': ['-std=c++11'],
            'darwin': ['-std=c++11'], # add ", '-g3', '-O0', '-DDEBUG=0', '-UNDEBUG'" for debugging
            'cygwin': ['-std=c++11'],
        }

        # Define an Extension object that describes the Antlr accelerator
        parser_ext = setuptools.Extension(
            # Extension name shall be at the same level as the sa_{{grammar_name|lower}}_parser.py module
            name='{{grammar_name|lower}}.parser.sa_{{grammar_name|lower}}_cpp_parser',

            # Add the Antlr runtime source directory to the include search path
            include_dirs=["src/{{grammar_name|lower}}/parser/cpp_src/antlr4-cpp-runtime"],

            # Rather than listing each C++ file (Antlr has a lot!), discover them automatically
            sources=get_files("src/{{grammar_name|lower}}/parser/cpp_src", "*.cpp"),
            depends=get_files("src/{{grammar_name|lower}}/parser/cpp_src", "*.h"),
    ...
    setuptools.setup (
        ...
        version='0.0.1',
        description='Speedy {{grammar_name}}',
...
if not using_fallback:
    try:
        ...
        if '{{grammar_name|lower}}_EXAMPLE_REQUIRE_CI_BINARY_BUILD' in os.environ:
